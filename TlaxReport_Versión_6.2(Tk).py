...
def guardar_denuncia():
    ...
    if contador >= 100:
        messagebox.showwarning(
            "Límite alcanzado",
            "Ya no se pueden registrar más denuncias."
        )
        return
    ...
    if tipo == "1":
        ...
        tipoDenuncia[contador] = "Sin especificar"
    ...
    print(tipoDenuncia[:contador])


def mostrar_denuncias():
    print("Entró a mostrar denuncias")
...
def buscar_nombre():
    ...
    buscar = entrada_buscar_nombre.get().strip()
    print("Buscando nombre: ", buscar)
    print("Lista: ", nombre[contador])

    for i in range(contador):

        print("Comparando: ", nombre[i])
    ...
    resultado.delete("1.0", tk.END)
    resultado.insert(tk.END, "No encontrado")

def buscar_tipo():
    print("Entró a buscar tipo")

    tipo = entrada_buscar_tipo.get().strip()
    ...
    for i in range(contador):
        ...
        print("Comparando: ", tipoDenuncia[i])
...
opcion_tipo = tk.Entry(ventana, width=40)
